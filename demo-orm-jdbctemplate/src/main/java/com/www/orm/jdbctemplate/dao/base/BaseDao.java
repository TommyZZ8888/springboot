package com.www.orm.jdbctemplate.dao.base;
...
import com.www.orm.jdbctemplate.annotation.Column;
import com.www.orm.jdbctemplate.annotation.Ignore;
import com.www.orm.jdbctemplate.annotation.PK;
import com.www.orm.jdbctemplate.annotation.Table;
...
public class BaseDao<T, P> {
    ...
    protected Integer updateById(T t, P pk, Boolean ignoreNull) {
        ...
        List<String> columns = columnList.stream().map(s -> StrUtil.appendIfMissing(s, " = ?")).toList();
        String params = StrUtil.join(Const.SEPARATOR_COMMA, columns);

        // 构造值
        List<Object> valueList = filterField.stream().map(field -> ReflectUtil.getFieldValue(t, field)).toList();
        valueList.add(pk);
    ...
    public List<T> findByExample(T t) {
        ...
        List<String> columns = columnList.stream().map(s -> " and " + s + " = ? ").toList();
    ...
    private List<Field> getField(T t, Boolean ignoreNull) {
        ...
        // 是否过滤字段值为null的字段
        if (ignoreNull) {
            filterField = fieldStream.filter(field -> ObjectUtil.isNotNull(ReflectUtil.getFieldValue(t, field))).toList();
        } else {
            filterField = fieldStream.toList();
        }
        return filterField;
    }

...
public class MultiThreadDownLoad {
    ...
    public MultiThreadDownLoad(URI uri, String localPath, long length) {
        this.uri = uri;
    ...
    public boolean start() {
        try {
            ...
            raf.close();
            int count = (int) Math.ceil((double) this.length / blockSize);
            this.latch = new CountDownLatch(count);
            log.info("MultiThreadDownLoad start file size " + this.length + " Thread number " + count);
    ...
    public class DownloadThread extends Thread {
        ...
        public DownloadThread(long threadId, long startIndex, long endIndex) {
            this.get = new HttpGet(MultiThreadDownLoad.this.uri);
        ...
        public void run() {
            ...
            try {
                try (CloseableHttpClient client = HttpClients.createDefault()) {
                    ...
                    raf = new RandomAccessFile(MultiThreadDownLoad.this.localPath, "rwd");
                ...
                e.printStackTrace();
                MultiThreadDownLoad.this.status.set(false);
                MultiThreadDownLoad.this.exit();

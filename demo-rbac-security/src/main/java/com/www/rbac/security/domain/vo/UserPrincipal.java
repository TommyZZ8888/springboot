...
public class UserPrincipal implements UserDetails {
    ...
    public static UserPrincipal create(User user, List<Role> roles, List<Permission> permissionList) {
        List<String> roleNames = roles.stream().map(Role::getName).collect(Collectors.toList());

        List<GrantedAuthority> authorities = permissionList.stream().filter(permission -> StrUtil.isNotBlank(permission.getPermission()))
